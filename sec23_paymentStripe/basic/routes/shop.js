...
router.get('/products/:productId', isAuth, shopProduct.getProduct);

router.get('/products', isAuth, shopProduct.getProductList);
...
router.get('/cart', isAuth, shopProduct.getCart);

// /*post can sent data in req.body*/
router.post('/cart', isAuth, shopProduct.postCart);

// /*in cart page press order-now move everything to order page*/
//router.post('/create-order', isAuth, shopProduct.postOrder);

router.post('/cart-delete-item', isAuth, shopProduct.postCartDeleteProduct)

router.get('/checkout', isAuth, shopProduct.getCheckout);
/*stripe cancel or failed redirect to checkout page again*/
router.get('/checkout/cancel', isAuth, shopProduct.getCheckout); 
/*when payment complt then post content to order collection , we're we can collect invoice*/
router.get('/checkout/success', isAuth, shopProduct.postOrder); 

router.get('/orders', isAuth, shopProduct.getOrdersList);

router.get('/orders/:orderId', isAuth, shopProduct.getInvoice);
